...
Obrolan Timor adalah aplikasi chat yang dikembangkan menggunakan Python, JavaScript, HTML, dan CSS.
...
1. Clone repositori ini:
    ```sh
    git clone https://github.com/Kirenius-kollo/Obrolan-Timor.git
    cd Obrolan-Timor
    ```
...
3. Inisialisasi aplikasi di Elastic Beanstalk:
    ```sh
    eb init -p python-3.x obrolan-timor
    ```

4. Buat dan deploy aplikasi:
    ```sh
    eb create obrolan-timor-env
    eb deploy
    ```
...
`.github/workflows/deploy.yml` dengan konten berikut:
    ...
    steps:
      ...
      - name: Deploy to Heroku
        env:
          HEROKU_API_KEY: ${{ secrets.6379ff29-1bb9-7d54-009f-486bf8d8a209 }}
        run: |
          git remote add heroku https://git.heroku.com/obrolan-timor.git
          git push heroku main}}
        run: |
